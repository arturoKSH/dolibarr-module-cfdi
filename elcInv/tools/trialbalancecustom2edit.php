 ...
 */
 ...
 if (GETPOST('button_removefilter_x', 'alpha') || GETPOST('button_removefilter.x', 'alpha') || GETPOST('button_removefilter', 'alpha')) {
    $search_accountancy_code_start = 0;
    $search_accountancy_code_end = -1;
    $moreforfilter1 = '';
    $search_date_start = '';
    $search_date_end = '';
    $filter = array();
...
$total_sous = 0;
$Saldo_Iniciall=0;
if(empty(GETPOST('date_start'))){
  $_POST['date_start'] = "01/01/2022";
    //GETPOST('date_start') = "01/01/".date("Y");
}
if(empty(GETPOST('date_end'))){
    $_POST['date_end'] = "31/12/".date("Y");
}
if(!empty($_GET['date_startday'])){
  //  GETPOST('date_start')=$_GET['date_start'];
    //GETPOST('date_end')=$_GET['date_end'];
}
...
$result = dad($db, $search_accountancy_code_start, $search_accountancy_code_end);
if (!empty($result)) {
    $contPrin=0;
    $saldoInFinallTT  = 0;
    $dfinallT  = 0;
    ...
    foreach ($result as $key => $value) {
        ...
        if (!empty(GETPOST('date_start')) && !empty(GETPOST('date_end'))) {
            $Vtree = treedate($db, $value['NumCta'], '', $datest, $dateen, $search_accountancy_code_end, $search_accountancy_code_start);
            $TParent = totaldate($db, $value['NumCta'], $datest, $dateen, $search_accountancy_code_end, $search_accountancy_code_start);
            ...
            foreach ($Vtree as $key => $value2) {
                ...
                if ($exist) {
                 //   echo  $value2['Debe'].'+<br>';
                    ...
                    // Es padre
                   // var_dump($TParent);
                    //echo $value2['Debe'] + $TParent[$key]['Debe'].'-<br>';
                    $tot[] = [
        ...
        if (!empty($Vtree)) {
            ...
            //    echo $value2['Index']."--";
                 ...
                 if(!empty($verifi[0]['account_number'])){
                        ...
                        $dfinallI=$deb;
                        $hfinallI= $hab;
                        $finalFFI=($Saldo_Inicialh+$deb)-($hab);
                    }else{
        ...
        print '<td class="right"></td>';
        //echo $dfinall.'<br>';
        $saldoInFinallTT  = $saldoInFinallTT + $saldoInFinall;
        $dfinallT  = $dfinallT + $dfinall + $dfinallD ;
        $hfinallT  = $hfinallT + $hfinall;
        $finalFFT  = $finalFFT + $finalFF;

// echo $dfinallT . 'values dfinallt'. $dfinall . '----' .$hfinallt;
                                    ...
                                    if (!empty($Vtree)) {
                                        ...
                                        foreach ($Vtree as $key => $value2) {
                                            ...
                                            if (!empty($exist) && $exist == 'true') {
                                                ...
                                                // $resN = $db->query($queryN);
                                               // $resultN = $resN->fetch_all(MYSQLI_ASSOC);
                                               $query = "SELECT * FROM llx_accounting_account where fk_pcg_version = 'crasaOficial' order by rowid asc";
$result =  $db->query($query);

// Define un array para almacenar las cuentas organizadas por su cuenta padre
$cuentas_organizadas = array();

// Recorre los resultados de la consulta y organiza las cuentas en un array multidimensional
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Si no hay cuenta padre, coloca esta cuenta directamente en el nivel superior
    if ($account_parent === null) {
        $amount_t = 0;
       // echo     $label = $row['label'].'<br>';
        $amount_t = getsumLevel($account_number);
        $tt_debit =  $tt_debit +  $amount_t;
        $next_account = getaccountNextLavel($rowid);
        if($next_account){
            foreach($next_account as $key => $value){
                $amount_t = 0;
                $next_account2 = getaccountNextLavel($value['rowid']);
                $amount_t = getsumLevel($value['account_number']);
                $tt_debit =  $tt_debit +  $amount_t;
                //echo ' &nbsp; --('.$value['account_number'].')'.$value['label'].'->'.$amount_t.'<br>';
               // echo $amount_t.'<br>';
                if($next_account2){
                    foreach($next_account2 as $key2 => $value2){
                        $amount_t = 0;
                        $amount_t = getsumLevel($value2['account_number']);
                        $tt_debit =  $tt_debit +  $amount_t;
                       // echo '&nbsp;&nbsp;**('.$value2['account_number'].')'.$value2['label'].'->'.$amount_t.'<br>';
                        // echo'  **'.$value2['label'].'<br>';
                       // echo $amount_t.'<br>';
                    }
                }
       ...
       echo '<br>';
        ...
        // Si hay cuenta padre, agrega esta cuenta como hijo de la cuenta padre correspondiente
       
    }
}
echo 'totalll'. $tt_debit;
function getsumLevel($numero_compte){
    global $db;
     $query_transacciones = "SELECT a.rowid AS rowid_cuenta, a.account_number , SUM(ifnull(b.debit,0)) AS debit
             FROM llx_accounting_account a
               LEFT JOIN llx_accounting_bookkeeping b ON a.account_number = b.numero_compte
               where b.doc_date between '2024-01-01' and '2024-01-31'
               and  b.numero_compte = '".$numero_compte."'
               GROUP BY a.rowid, a.account_number ";
    $result_transacciones = $db->query($query_transacciones);
    // Recorre los resultados y actualiza las cuentas en el array con los totales de transacciones
...
function getaccountNextLavel($parent_account){
    ...
    return $result;
    //$result = $db->query($query_transacciones);
}
                                                ...
                                                if (!empty($resultN)) {
                                                //     print '<tr><td colspan="6"><ul class="collapse list-unstyled" id="Padre2'.$cont.'">
                                                //             <li>
                                    ...
                                    /*print '<tr class="liste_total">
                                                ...
                                                <td class="nowrap right">' . $debe . '</td>
                                                <td class="nowrap right">' . $haber . '</td>
                                                <td class="nowrap right">' . $total . '</td>';
                                    print "<td>&nbsp;</td>\n";
                                    print '</tr>';*/
    print'                      </table>
                            </li> 
...
print '<tr class="liste_total">
    ...
    ';
    print '<td class="nowrap right">' . price($dfinallT) . '</td>';
    print '<td class="nowrap right">' . price($hfinallT) . '</td>';
    print '<td class="nowrap right">' . price($finalFFT) . '</td>';
print "<td>&nbsp;</td>\n";
print '</tr>';
