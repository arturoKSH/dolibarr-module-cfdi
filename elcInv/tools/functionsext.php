...
function dad($db = null)
{
    ...
    $query .= "and     a.account_parent = b.rowid  ";
    //$query .= "where  b.account_number is null ";
...
function accountsTree($datest,$dateen,$search_accountancy_code_start,$search_accountancy_code_end){
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (!$account_parent) {
            ...
            $next_account = getaccountNextLavel($rowid,$search_accountancy_code_start,$search_accountancy_code_end);
            if($next_account){
                foreach($next_account as $key => $value){
                    $amount_t = 0;
                    $tt_debit_by_parent_level2 = 0;
                    $tt_credit_by_parent_level2 = 0;
                    $next_account2 = getaccountNextLavel($value['rowid'],$search_accountancy_code_start,$search_accountancy_code_end);
...
function getaccountNextLavel($parent_account,$search_accountancy_code_start = '',$search_accountancy_code_end = ''){
    global $db;
    $result= '';
    $query = "SELECT * FROM llx_accounting_account where account_parent = ".$parent_account;
    if($search_accountancy_code_start) 
        $query .= " and account_number >= '".$search_accountancy_code_start."'";
    if($search_accountancy_code_end)  
        $query .= " and account_number <= '".$search_accountancy_code_end."'";
