<?php 
	...
	public  function generar($id,$outputlangs,$db,$ref, $xml){
		...
		$file = $dir.'/'.$ref."-SolicitudCancel.pdf";

		
		//$file = DOL_DATA_ROOT."/probando.pdf";
		...
		$pdf->SetFont('', '', 10);
		$pdf->MultiCell(120, 3, $xml['RfcEmisor'], 0, 'L', 0);
		...
		$pdf->MultiCell($widthrecbox + 15, $hautcadre, $statusCanel[strval($xml->Folios->EstatusUUID)], 0, 'L');
...
/*if ($showaddress)
		...
		// $factuas = " SELECT a.rowid,a.fk_mode_reglement, c.mofpaymt, d.serie,f.ref ";
 		// $factuas.= " FROM ".MAIN_DB_PREFIX."facture a left join ".MAIN_DB_PREFIX."facturedet b";
 		// $factuas.="  on a.rowid=b.fk_facture left join ".MAIN_DB_PREFIX."facture_extrafields c ";
 		// $factuas.="  on a.rowid=c.fk_object left join ".MAIN_DB_PREFIX."serie d ";
		...
		// $file = $dir.'/'.$objfactuas[0]['ref'].".xml";
			...
			// if (empty($doc->getElementsByTagName('Complemento')->item($count))) {
				

			// 	unset($_SESSION["timbrar"]);
			// }else{
			// 	$RfcEmisor = $doc->getElementsByTagName('Complemento')->item($count);
...
/*}*/
		...
		{*/
			...
			$pdf->SetXY($posx+68, $posy-20);
			$pdf->SetFont('', 'B', $default_font_size+2);
			//$pdf->MultiCell($widthrecbox+10, 4, 'Acuse de cancelación de CFDI ', 0, 'L');
			$pdf->MultiCell($widthrecbox+10, 4, 'Acuse de cancelación de CFDI ', 0, 'L');
