...
class PrivateKey extends Key
{
     ...
     */
    public function __construct($source, $passPhrase)
    {
        if ('' === $source) {
     ...
     * @return string
     */
    public static function convertDerToPem($contents, $isEncrypted)
    {
     ...
     * @return self
     */
    public static function openFile($filename, $passPhrase)
    {
    ...
    public function sign($data, $algorithm = OPENSSL_ALGO_SHA256)
    {
        $return = $this->callOnPrivateKey(
            function ($privateKey) use ($data, $algorithm) {
                $result = $this->openSslSign($data, $signature, $privateKey, $algorithm);
                if (false === $result[0]) {
     ...
     * @internal
     */
    protected function openSslSign($data, $signature = null, $privateKey, $algorithm)
    {
        //bools(true) // asignature
        $result = openssl_sign($data, $signature, $privateKey, $algorithm);
        return [$result, $signature];
    }

    public function belongsTo($certificate)
    {
        return $this->belongsToPEMCertificate($certificate->pem());
    }

    public function belongsToPEMCertificate($certificate)
    {
        return $this->callOnPrivateKey(
            function ($privateKey) use ($certificate) {
     ...
     */
    public function callOnPrivateKey($function)
    {
        ...
        try {
            $result = call_user_func($function, $privateKey);
            return $result;
        } finally {
    ...
    public function changePassPhrase($newPassPhrase)
    {
        $pem = $this->callOnPrivateKey(
            function ($privateKey) use ($newPassPhrase) {
                $exportConfig = [
