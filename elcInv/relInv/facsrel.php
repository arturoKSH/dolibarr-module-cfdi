...
if ($action=='delete') {
    ...
    $query= " delete  from ".MAIN_DB_PREFIX."KSHCFDIRelDtl where rowid =".$_GET['fk_facrel'];
    
    $res = $db->query($query);
}
if (isset($_POST['actionaddrel'])) {
    
    echo "pruebas ".$_POST['typerelnew'];
    $query= " Insert into ".MAIN_DB_PREFIX."KSHCFDIRelDtl (fk_facture_parent,reltype,uuid) ";
...
$queryfacsRel.=" from  ".MAIN_DB_PREFIX."KSHCFDIRelDtl a left join ".MAIN_DB_PREFIX."facture_extrafields b ";
...
//SATKSH
$query = "SELECT * FROM ".MAIN_DB_PREFIX."KSHRelTypeTbl";
$res = $db->query($query);
...
$typerelsat .= " where  a.fk_soc= '" . $socid . "' and (b.uuid is not null and b.uuid != '".$object->array_options['options_uuid']."') and a.type = 0 ";
//$typerelsat .= " where  a.fk_soc= '" . $object->thirdparty->getNomCustom(1, 'customer') . "' and b.uuid is not null and a.type = 0 ";
//echo $typerelsat;
        ...
        print '<div class="div-table-responsive-no-min">';
            print '<table class="noborder centpercent">';
                ...
                print '<tr class="oddeven nodrag nodrop nohover">';
                    ...
                    print '</td>';
                    print '<td colspan="3" class="center">';
                      print '<select name="typerelnew" class="flat selectpaymenttypes">';
                      //print_r($valselect);
                        foreach ($valselect as $key => $value) {
                            print '<option value="'.$value['RelTypId'].'">'.$value['Description'].'</option>';
                        }
                    print '</select>';
